...
// TODO use options from get_options_local_storage
...
function export_pdf(render_options) {
    ...
    var doc = new jsPDF({
        unit: "px",
        format: download_options.format.toLowerCase(),
        orientation: download_options.orientation.toLowerCase(),
    });
    var doc_width = doc.internal.pageSize.getWidth();
    var doc_height = doc.internal.pageSize.getHeight();
    var doc_in = new jsPDF({
        unit: "in",
        format: download_options.format.toLowerCase(),
        orientation: download_options.orientation.toLowerCase(),
    });
    var margin =
        (download_options.margin / doc_in.internal.pageSize.getWidth()) *
        doc_width;
    if (margin == 0) {
        prism_container.style.borderRadius = "0px";
        prism_output.style.borderRadius = "0px";
    ...
    if (
        parseInt(getComputedStyle(prism_output).height) >
        doc_height - margin * 2
    ) {
        ...
        htmlToImage
            ...
            .then(function (dataUrl) {
                ...
                doc.addImage(
                    ...
                    use_width / 2,
                    use_height / 2
                ); // Divide by 2 because original use_width variable multiplies the doc by 2
                if (index == last) {
                    function cleanup() {
                        doc.save(filename);
                        var lines = Array.from(
                            prism_output.children[0].children[0].children
                        );
            ...
            .catch(function (error) {
                console.error(
                    "Something went wrong with exporting PDF!",
                    error
                );
            });
    }
...
function calc_lines_per_page(page_height) {
    ...
    var line_height = parseInt(
        getComputedStyle(prism_output.children[0].children[0].children[0])
            .height
    );
    var lines = Array.from(prism_output.children[0].children[0].children);
    var first_line = 1;
...
function open_download_container() {
    ...
    current_percent = 2;
    progress_file.textContent =
        download_options.name + "." + download_options.file;
}
